{{-- ════════════════════════════════════════════════════════════
FILE: resources/views/cart/payment.blade.php
════════════════════════════════════════════════════════════ --}}
@extends('layouts.app')
...
<div class="pay-card">
    ...
    <div class="pay-header">
        ...
        <div class="pay-amount">₱{{ number_format($total, 2) }}</div>
        <div class="pay-sub">Ferrari System Secure Checkout</div>
    </div>

    <form method="POST"
          action="{{ route('cart.process-payment') }}"
          id="cardForm"
        ...
        <button type="submit" class="btn-pay-now" id="payNowBtn">
            ...
            <span id="payLabel">
                <i class="fas fa-lock" style="margin-right:6px"></i>Pay ₱{{ number_format($total, 2) }}
            </span>
        </button>
    </form>
    ...
    <div class="pay-header">
        ...
        <div class="pay-amount">₱{{ number_format($total, 2) }}</div>
        <div class="pay-sub">Confirm your payment below</div>
    </div>
    ...
    <div class="paypal-mock-box">
        ...
        <div class="pp-amount">₱{{ number_format($total, 2) }}</div>
        <div class="pp-note">Ferrari System · Simulated PayPal Environment</div>
    </div>
...
<script>
/**
 * Called via onsubmit="return handleSubmit(...)" on the <form>.
 * Shows the spinner and disables the button, then returns TRUE
 * so the browser continues with the POST submission.
 *
 * @param {string}  btnId    — id of the submit button
 * @param {string}  labelId  — id of the text label inside the button
 * @param {boolean} isPayPal — hides the PayPal icon when true
 */
function handleSubmit(btnId, labelId, isPayPal = false) {
    ...
    // Hide PayPal icon so spinner has space
    if (isPayPal) {
        const icon = document.getElementById('ppIcon');
        if (icon) icon.style.display = 'none';
    }

    return true; // ← CRITICAL: allows the form to actually submit
}
