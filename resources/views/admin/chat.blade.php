{{-- resources/views/admin/chat.blade.php — Admin ↔ Driver --}}
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Chat with Driver — Admin</title>
...
.topbar{background:var(--dark2);border-bottom:1px solid rgba(220,0,0,.15);padding:0 20px;height:56px;display:flex;align-items:center;gap:12px;flex-shrink:0}
...
.badge-driver{background:rgba(68,136,255,.12);border-color:#4488ff;color:#4488ff}
...
<body>
<div class="topbar">
    <a href="{{ route('admin.orders.show', $order) }}" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Order</a>
    <div>
        <div class="chat-title">
            <i class="fas fa-truck" style="color:#4488ff;margin-right:5px"></i>
            {{ $order->driver->name ?? 'No Driver Assigned' }}
            <span class="badge badge-driver" style="margin-left:6px">Driver</span>
        </div>
        <div class="chat-subtitle">
            Order #{{ $order->id }} · {{ $order->car->name ?? '' }} · Customer: {{ $order->user->name ?? '' }}
        </div>
    </div>
</div>
...
<div class="messages-wrap" id="msgWrap">
    ...
    @empty
        <div class="empty"><i class="fas fa-comments"></i>No messages with driver yet.</div>
    @endforelse
</div>

<div class="input-area">
    <form method="POST" action="{{ route('admin.chat.send', $order) }}" style="display:contents">
        @csrf
        <textarea name="body" placeholder="Message driver..." required
...
setInterval(() => {
    fetch('{{ route('admin.chat.poll', $order) }}')
