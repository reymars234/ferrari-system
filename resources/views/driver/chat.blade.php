{{-- resources/views/driver/chat.blade.php — Driver ↔ Customer --}}
<!DOCTYPE html>
<html lang="en">
<head>
...
.badge-customer{background:rgba(29,185,84,.12);border-color:#1db954;color:#1db954}
...
.switch-admin{background:rgba(220,0,0,.1);border-color:rgba(220,0,0,.4);color:var(--red)}
.switch-admin:hover{background:var(--red);color:#fff}
    ...
    <div class="topbar-left">
        ...
        <div>
            <div class="chat-title">
                <i class="fas fa-user" style="color:#1db954;margin-right:5px"></i>
                {{ $order->user->name ?? 'Customer' }}
                <span class="badge badge-customer" style="margin-left:6px">Customer</span>
            </div>
            <div class="chat-subtitle">Order #{{ $order->id }} · {{ $order->car->name ?? '' }}</div>
        </div>
    </div>
    <a href="{{ route('driver.admin-chat', $order) }}" class="switch-btn switch-admin">
        <i class="fas fa-user-shield"></i> Chat with Admin
    </a>
</div>
...
<div class="messages-wrap" id="msgWrap">
    ...
    @empty
        <div class="empty"><i class="fas fa-comments"></i>No messages yet. Say hello!</div>
    @endforelse
</div>

<div class="input-area">
    <form method="POST" action="{{ route('driver.chat.send', $order) }}" style="display:contents">
        @csrf
        <textarea name="body" placeholder="Message customer..." required
...
setInterval(() => {
    fetch('{{ route('driver.chat.poll', $order) }}')
